...
- дублей по ссылке в результате нет; одно совпадение по названию («The Star-Touched Queen») — это два разных товара с разной ценой и страницей, поэтому дубли проверяю по ссылке, а не по названию;
...
- **Если страница не загрузилась после повторных попыток или пришла без товаров**, категория помечается как неполная. В итоговом отчёте видно, где и до какой страницы данные собраны, а скрипт завершается с кодом 1, чтобы сбой не остался незамеченным.
- **Отсутствующие поля** на карточке не роняют скрипт: в текстовых полях подставляется «не указано», нераспознанная цена остаётся пустой ячейкой.
...
- Дубли скрипт не удаляет: товар может законно лежать в нескольких категориях.
